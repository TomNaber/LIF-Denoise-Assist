...
def lif_rebuilder(LIF_files, tiff_files, channel_colors, progressbar, progress_label, file_label, output_dir,
    ...
    for LIF in LIF_files:
        ...
        with open(metadata_dir + "/" + LIF.split("/")[-1] + '.json') as json_file:
            # Load the contents of the file
            metadata = json.load(json_file)

        metadata["axes"] = 'ZCYX'
        metadata['PhysicalSizeX'] = 1 / float(metadata['scale_n']['1'])
        metadata['PhysicalSizeY'] = 1 / float(metadata['scale_n']['2'])
        metadata['PhysicalSizeZ'] = 1 / float(metadata['scale_n']['3'])
        metadata['Channel'] = {"Name": active_colors}
        ...
        filename = output_dir + LIF.split(input_dir)[1] + " Denoised.tiff"
        tifffile.imwrite(filename, stacked, metadata=metadata, ome=True)
...
def main(metadata_dir):
    ...
    lif_rebuilder(LIF_files, tiff_files, channel_colors, progressbar, progress_label, file_label, output_dir, input_dir, metadata_dir)
    root.destroy()
    return
