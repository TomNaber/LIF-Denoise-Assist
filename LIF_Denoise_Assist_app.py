...
def main():
    ...
    total_slices, total_stacks, _, _, metadata = variable_initialization(input_dir, liffiles)
    total_slices, total_stacks = slice_counter(liffiles, input_dir, total_stacks, total_slices)
    create_gui(metadata, 0, total_stacks)
    progress_label = st.empty()
    file_label = st.empty()

    # Process LIF files
    process_lif_files(liffiles, input_dir, output_dir, total_slices, total_stacks, progress_label, file_label)
